...
#press2=input('select the above')
	...
	if press=='3':
		   ...
		   money=(df.Price[df['serialno']==order])+100
		   print(money)
		   con=input('do you want to place order(y/n) ')
		   print('---------------------------------------------------------------------------------------------------------------------------------------')
		   data=pd.DataFrame({'NAME':Name,'Phoneno':phoneno,'Address':Address,'emailid':emailid,'money':money,})
	...
	if press=='2':
	   	...
	   	... 
	   	if filter2=='o':
	   		print(df[df.BRAND=='Oppo'])
	   	...
	   	if filter2=='v':
	   		print(df[df.BRAND=='Vivo'])
	   	...
	   	if filter2=='p':
	   		print(df[df.BRAND=='Oneplus'])
	if press=='5':
	   df1=pd.read_csv('NAME14.csv')
	   hr=data.to_csv('NAME14.csv',',')
	   print(NAME14.csv)
